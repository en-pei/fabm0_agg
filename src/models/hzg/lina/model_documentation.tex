...
\begin{table*}[h]
  ...
  \hline
    ...
    $Q^*_N$ & Maximum nitrogen to carbon ratio & mol-N mol-C$^{-1}$ \\
    $Q^*_P$ & Maximum phosphorus to carbon ratio & mol-P mol-C$^{-1}$ \\
    ...
    $\alpha$ & Photosynthesis slope &  \\
    $\mu_{\textrm{max}}$ & Maximum growth rate & d$^{-1}$ \\
    $E$ & Exopolymer material (EPS) production & mol-C m$^{-3}$ \\
    $E_{\textrm{min}}$ & Minimum EPS production & mol-C m$^{-3}$ \\
    $E_{\textrm{max}}$ & Maximum EPS production & mol-C m$^{-3}$ \\
    $\dot{c}$ & Physiology stress for nutrient limitation & d$^{-1}$ \\
    $B$ & Parameter for EPS production & -- \\
    $B^*$ & Other parameter for EPS production & d \\
    $n^*$ & Intrinsec metabolic independence & -- \\ \hline
