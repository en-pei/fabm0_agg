...
filelist=[]
path='/gpfs/work/lie/run/par/test'
...
##define dataset
tsmdatat=oszspmmay[:,0]+6-0.3 #tmzspmjuly[:,0]+7.5 # bale3500spmc[:,0]-0.95-0.1-0.6 #bale3500spmc[8:,0]-0.95-0.1-0.6 # +7 #tmzspmjuly[:,0]+7.5 #-0.42 #datatime #oszspmfeb[:,0] #tmzspmfeb[:-1,0]+4+6 #

d50datat=oszd50may[:,0]+6-0.3 #tmzd50july[:,0]+7.5 #bale3500d50[:,0]-0.95-0.1-0.6 #bale3500d50[8:,0]-0.95-0.1-0.6 #+7 #tmzd50july[:,0]+7.5 #-0.42 #s1datatime #oszd50feb[:,0] #tmzd50feb[:,0]+4+6 #
flowdatat=oszflowmay[:,0]+6-0.3 #tmzflowjuly[:,0]+7.5 #bale3500spmcf[:,0]-0.95-0.1-0.6#bale3500d50f[8:,0]-0.95-0.1-0.6# +7 #tmzflowjuly[:,0]+7.5 #-0.42 #oszflowfeb[:,0] #tmzflowfeb[:,0]+4+6 #
#flowdatat1=bale3500spmcf[:,0]-0.95-0.1 #-0.42

tsmdata=oszspmmay[:,1] #tmzspmjuly[:,1] #bale3500spmc[0:,1] ##bale3500spmc[8:,1] #tmzspmjuly[:,1] #dataspmc #oszspmfeb[:,1] #tmzspmfeb[:-1,1] #
d50data=oszd50may[:,1] #tmzd50july[:,1] #bale3500d50[0:,1] ##bale3500d50[8:,1] #tmzd50july[:,1] #s1datasize #oszd50feb[:,1]  #tmzd50feb[:,1] #
flowdata=oszflowmay[:,1] #tmzflowjuly[:,1] #bale3500spmcf[0:,1] ##bale3500d50f[8:,1] #tmzflowjuly[:,1]#oszflowfeb[:,1] #tmzflowfeb[:,1] #
...
fig, axs = plt.subplots(5, sharex=True)  #4 (5, sharex=False)
...
#		axs[2].plot(secs/3600,uu,label=i.name) #,color='seagreen') #color='firebrick',label="model"    u change with time alpha=0.3,  #abs(uu)*sqrt(2)
		...
		axs[4].plot(secs/3600,ws*1000,label=i.name) #xsize   plotting compound of all dD terms
...
axs[4].set_xlabel('time (h)',fontsize=20 )#('G')
...
axs[4].set_ylabel('w$_s$ (mm/s)',fontsize=20 )#('ESD ($\mu$m)') velocity  #'flow (m s$^{-1}$)'
plt.legend()
