...
path='/gpfs/work/lie/run/par/6tmzfeb'  #'/gpfs/work/lie/run/par/phy'#  linked best run from parameterauto.sh
...
##define dataset
tsmdatat=tmzspmfeb[:-1,0]+4+6+0.72 #tmzspmjuly[:,0]+7.5 # bale3500spmc[:,0]-0.95-0.1-0.6 #bale3500spmc[8:,0]-0.95-0.1-0.6 # oszspmmay[:,0]+6-0.3 #+7 #tmzspmjuly[:,0]+7.5 #-0.42 #datatime #oszspmfeb[:,0] #

d50datat=tmzd50feb[:,0]+4+6+0.72 #tmzd50july[:,0]+7.5 #bale3500d50[:,0]-0.95-0.1-0.6 #bale3500d50[8:,0]-0.95-0.1-0.6 #oszd50may[:,0]+6-0.3 #+7 #tmzd50july[:,0]+7.5 #-0.42 #s1datatime #oszd50feb[:,0] #
flowdatat=tmzflowfeb[:,0]+4+6+0.72 #tmzflowjuly[:,0]+7.5 #bale3500spmcf[:,0]-0.95-0.1-0.6#bale3500d50f[8:,0]-0.95-0.1-0.6# oszflowmay[:,0]+6-0.3 #+7 #tmzflowjuly[:,0]+7.5 #-0.42 #oszflowfeb[:,0] #
#flowdatat1=bale3500spmcf[:,0]-0.95-0.1 #-0.42

tsmdata=tmzspmfeb[:-1,1] #tmzspmjuly[:,1] #bale3500spmc[0:,1] ##bale3500spmc[8:,1] #oszspmmay[:,1] #tmzspmjuly[:,1] #dataspmc #oszspmfeb[:,1] #
d50data=tmzd50feb[:,1] #tmzd50july[:,1] #bale3500d50[0:,1] ##bale3500d50[8:,1] #oszd50may[:,1] #tmzd50july[:,1] #s1datasize #oszd50feb[:,1]  #
flowdata=tmzflowfeb[:,1] #tmzflowjuly[:,1] #bale3500spmcf[0:,1] ##bale3500d50f[8:,1] #oszflowmay[:,1] #tmzflowjuly[:,1]#oszflowfeb[:,1] #
...
#		Derosion=squeeze(ncv['agg_dflux'][:])
		...
		axs[0].plot(secs/3600,ctot,label=i.name) #,color='darkgreen') #color='firebrick', totlpm    #+0.27 #SPMC	vs t
		#axs[0].plot(secs/3600,phy+det,label=i.name) #totlpm    #+0.27 #SPMC	vs t
		#axs[0].plot(secs/3600,ctot1) #totlpm    #+0.27 #SPMC	vs t
		axs[1].plot(secs/3600,esd*1e6,label=i.name) #,color='darkgreen') #color='firebrick',+0.27#in m #ESD t  
...
#		axs[2].plot(secs/3600,G,label=i.name) #abs(uu)*1.414  u change with time alpha=0.3,
		...
		axs[2].plot(secs/3600,uu,label=i.name) #,color='darkgreen') #color='firebrick',label="model"    u change with time alpha=0.3,  #abs(uu)*sqrt(2)
...
axs[2].plot(flowdatat,flowdata,'o',color='grey',alpha=0.5,label='TMZ Feb') #'F&L TMZ July' /100 for labdata
