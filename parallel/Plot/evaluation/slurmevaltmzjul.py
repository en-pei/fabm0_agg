...
####log file for parameters####### tmzjulnew0  tmzjulnew0_+4-5  tmzjulnew2
dfp = pd.read_csv('/gpfs/work/lie/results/tmzjulnew0_+4-5/current.log', sep="-",header=None)#, names=['id','param','value']) #parameters       run/test/ bigrun numpy.linalg.LinAlgError: 2-th leading minor of the array is not positive definite
#currentsample.log
dfp.columns = ['id','param','value']
sdfp = dfp.sort_values(by=['id'], ascending=True) #sorted
#idp0 = sdfp['id'].values[0]
idp=sdfp['id'].to_numpy()
p0raw=sdfp.loc[sdfp['param'] == 'breakup_factor']  # 'pws' extract all parameter 0
p1raw=sdfp.loc[sdfp['param'] == 'coagulation_rate'] #'kws'
p2raw=sdfp.loc[sdfp['param'] == 'ks']   #'dens_lpm'
###add more parameter####
p3raw=sdfp.loc[sdfp['param'] == 'fractal_dimension1'] 
p4raw=sdfp.loc[sdfp['param'] == 'kbottom'] # 
p5raw=sdfp.loc[sdfp['param'] == 'tauc_const'] # 
...
#namelist=['pws','kws','ks','fractal_dimension1','kbottom','tauc_const','Xsize'] # 
...
dfe = pd.read_csv('/gpfs/work/lie/results/tmzjulnew0_+4-5/relativeerror.txt', sep=" ",header=None)#, names=['id','v2','v3','v4','v5','v6','v7']) #errors bigrun
#relativesample.txt
...
simple['sum']=simple['err_cr']*0.1+simple['err_dr']
#simple['mse']/40+simple['mses']/80 #+0.5*(simple['err_range'])
#print (simple)
#exit()

#bestinput=5000 #10000 #40000 #20
bestinput=round(0.001*len(simple))  #simple1 for lab #0.01
...
pvar0 ='v3' #input("Enter xaxis for plotting (v?): ") #'v0'
#pvar1 = input("yaxis (v?): ")
...
f=open('/gpfs/work/lie/results/median6.txt','w') ##'ab' 'w'
for i in medians:
  f.write(str(i)+'  ')#'\n'
#f.write(b'\n')
f.close()
...
np.savetxt('therun6.txt', therunpara, fmt='%s')
np.savetxt('paramtochange6.txt', therunpara, fmt='%s')
#print (list(data)) 
...
#	print (runnr, "best run id is: ",x)
	f=open('/gpfs/work/lie/results/paramtochange6.txt','ab')
...
for i,n in enumerate(names[0:6]): #[0:7] [2:6]
	print (n)
	...
	if n == pvar0:
		...
		#print (xt2min)
#		im=axs[i].tricontourf(xt2min[pvar0],xt2min[n],xt2min[ploterr]/xt2min[ploterr].max(),levels=50,cmap='YlGn_r', extend='max')#,, cmap='viridis_r')
...
#		im=axs[i].tricontourf(xre,yre,zre,levels=14,cmap='viridis_r') #interpolate data
		...
		#axs[i].scatter(therunpara[pvar0],therunpara[n],marker='^', c='red', s=30) #mark best run
