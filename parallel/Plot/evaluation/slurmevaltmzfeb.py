...
####log file for parameters#######  tmzfebnew2  tmzfebnew4smallkws
dfp = pd.read_csv('/gpfs/work/lie/results/tmzfebnew2/current.log', sep="-",header=None)#, names=['id','param','value']) #parameters       run/test/ bigrun numpy.linalg.LinAlgError: 2-th leading minor of the array is not positive definite
#currentsample.log
...
dfe = pd.read_csv('/gpfs/work/lie/results/tmzfebnew2/relativeerror.txt', sep=" ",header=None)#, names=['id','v2','v3','v4','v5','v6','v7']) #errors bigrun
#relativesample.txt
...
simple['sum']=simple['err_cr']*0.1+simple['err_dr'] #+simple['mse']+simple['mses'] #
...
pvar0 ='v0' #input("Enter xaxis for plotting (v?): ") #'v0'
#pvar1 = input("yaxis (v?): ")
...
###save median to file ### caustion to file name ####
...
np.savetxt('theruntmzfeb.txt', therunpara, fmt='%s')
np.savetxt('paramtochangetmzfeb.txt', therunpara, fmt='%s')
#print (list(data)) 

########added plotting the time series, call another python script############################################
print ("start plotting time series with data")
#subprocess.call("/gpfs/work/lie/bale2002/parameterautotmzfeb.sh",shell=True)  #!!!a different one!
...
#	print (runnr, "best run id is: ",x)
	f=open('/gpfs/work/lie/results/paramtochangetmzfeb.txt','ab')
