...
pvar0 ='v1' #'v3' #input("Enter xaxis for plotting (v?): ") #'v0'

####log file for parameters####### tmzjulnew0
dfp = pd.read_csv('/gpfs/work/lie/results/oszfebnew7/current.log', sep="-",header=None)#, names=['id','param','value']) #parameters       run/test/ bigrun numpy.linalg.LinAlgError: 2-th leading minor of the array is not positive definite
#currentsample.log
...
dfe = pd.read_csv('/gpfs/work/lie/results/oszfebnew7/relativeerror.txt', sep=" ",header=None)#, names=['id','v2','v3','v4','v5','v6','v7']) #errors bigrun
#relativesample.txt
...
simple['sum']=simple['err_dr']*0.1+simple['mses'] #simple['err_cr']#
#+simple['mse'] #+simple['mses'] #
#simple['mse']/40+simple['mses']/80 #+0.5*(simple['err_range'])
#print (simple)
#exit()

#bestinput=5000 #10000 #40000 #20
bestinput=round(0.001*len(simple))  #0.01   simple1 for lab
...
f=open('/gpfs/work/lie/results/medianoszfeb.txt','w') ##'ab' 'w'
for i in medians:
  f.write(str(i)+'  ')#'\n'
#f.write(b'\n')
f.close()
...
np.savetxt('therunoszfeb.txt', therunpara, fmt='%s')
np.savetxt('paramtochangeoszfeb.txt', therunpara, fmt='%s')
#print (list(data)) 

########added plotting the time series, call another python script############################################
print ("start plotting time series with data")
#subprocess.call("/gpfs/work/lie/bale2002/parameterautooszfeb.sh",shell=True)  #!!!a different one!
...
#	print (runnr, "best run id is: ",x)
	f=open('/gpfs/work/lie/results/paramtochangeoszfeb_.txt','ab') ####????????????????
...
#	print (runnr, "best run id is: ",x)
	f=open('/gpfs/work/lie/results/paramtochangeoszfeb.txt','ab')
	np.savetxt(f,runnr,fmt='%s',delimiter=' ')
	f.write(b'\n')
	f.close()
