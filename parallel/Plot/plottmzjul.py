...
filelist=[]
#path='/gpfs/work/lie/bale2002/par'
path='/gpfs/work/lie/run/par/6param'  #linked best run from parameterauto.sh
...
bale3500spmc=asarray(numpy.loadtxt('%s/newspmc_bale3500.txt'%datadir,delimiter=' '))  #', '
bale3500spmcf=asarray(numpy.loadtxt('%s/newflow_balespmc3500.txt'%datadir,delimiter=' '))
bale3500d50=asarray(numpy.loadtxt('%s/newd50_bale3500.txt'%datadir,delimiter=' '))
bale3500d50f=asarray(numpy.loadtxt('%s/flow_baled503500.txt'%datadir,delimiter=', '))#this is old


##define dataset
tsmdatat=tmzspmjuly[:,0]+7.5 # bale3500spmc[:,0]-0.95-0.1-0.6 #bale3500spmc[8:,0]-0.95-0.1-0.6 # oszspmmay[:,0]+6-0.3 #+7 #tmzspmjuly[:,0]+7.5 #-0.42 #datatime #oszspmfeb[:,0] #tmzspmfeb[:-1,0]+4+6 #

d50datat=tmzd50july[:,0]+7.5 #bale3500d50[:,0]-0.95-0.1-0.6 #bale3500d50[8:,0]-0.95-0.1-0.6 #oszd50may[:,0]+6-0.3 #+7 #tmzd50july[:,0]+7.5 #-0.42 #s1datatime #oszd50feb[:,0] #tmzd50feb[:,0]+4+6 #
flowdatat=tmzflowjuly[:,0]+7.5 #bale3500spmcf[:,0]-0.95-0.1-0.6#bale3500d50f[8:,0]-0.95-0.1-0.6# oszflowmay[:,0]+6-0.3 #+7 #tmzflowjuly[:,0]+7.5 #-0.42 #oszflowfeb[:,0] #tmzflowfeb[:,0]+4+6 #
#flowdatat1=bale3500spmcf[:,0]-0.95-0.1 #-0.42

tsmdata=tmzspmjuly[:,1] #bale3500spmc[0:,1] ##bale3500spmc[8:,1] #oszspmmay[:,1] #tmzspmjuly[:,1] #dataspmc #oszspmfeb[:,1] #tmzspmfeb[:-1,1] #
d50data=tmzd50july[:,1] #bale3500d50[0:,1] ##bale3500d50[8:,1] #oszd50may[:,1] #tmzd50july[:,1] #s1datasize #oszd50feb[:,1]  #tmzd50feb[:,1] #
flowdata=tmzflowjuly[:,1] #bale3500spmcf[0:,1] ##bale3500d50f[8:,1] #oszflowmay[:,1] #tmzflowjuly[:,1]#oszflowfeb[:,1] #tmzflowfeb[:,1] #
...
Gsidf=[]
filenumber=0
fig, axs = plt.subplots(3, sharex=True)  #4 (5, sharex=False)
rcParams['lines.linewidth']=7 #5 #3
rcParams['lines.color']='firebrick' #3
nn=7.55 #make the time start from 0h
...
#		Derosion=squeeze(ncv['agg_dflux'][:])
		...
		axs[0].plot(secs/3600-nn,ctot,label=i.name,color='darkgreen') #color='firebrick', totlpm    #+0.27 #SPMC	vs t
		#axs[0].plot(secs/3600,phy+det,label=i.name) #totlpm    #+0.27 #SPMC	vs t
		#axs[0].plot(secs/3600,ctot1) #totlpm    #+0.27 #SPMC	vs t
		axs[1].plot(secs/3600-nn,esd*1e6,label=i.name,color='darkgreen') #color='firebrick',+0.27#in m #ESD t  
...
#		axs[2].plot(secs/3600,G,label=i.name) #abs(uu)*1.414  u change with time alpha=0.3,
		...
		axs[2].plot(secs/3600-nn,uu,label=i.name,color='darkgreen') #color='firebrick',label="model"    u change with time alpha=0.3,  #abs(uu)*sqrt(2)
...
axs[0].plot(tsmdatat-nn,tsmdata,'o',color='k',markersize=9,mfc='grey',alpha=0.8) #,label='F&L TMZ July' 0.8
...
axs[0].set_ylabel('SPMC (g m$^{-3}$)', fontsize=20 )
...
axs[1].plot(d50datat-nn,d50data,'o',color='k',markersize=9,mfc='grey',alpha=0.8) #'none' ,label='F&L TMZ July'
...
axs[1].set_ylabel('size ($\mu m$)', fontsize=20 )
...
axs[2].plot(flowdatat-nn,flowdata,'o',color='k',markersize=9,mfc='grey',alpha=0.8,label='data') #'TMZ July' 'F&L TMZ July' /100 for labdata
#axs[2].fill_between(flowdatat,flowdatamax,flowdatamin,alpha=0.30, color='grey',label='data') #'tab:orange'  #linewidth=3,
axs[2].set_xlabel('time (h)',fontsize=20 )#('G')
axs[2].set_xlim(0,12.49)
axs[2].set_ylim(0,0.9)
axs[2].set_ylabel('flow (m s$^{-1}$)',fontsize=20 )#('ESD ($\mu$m)') velocity 
