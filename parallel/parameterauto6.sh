...
parampath="/gpfs/work/lie/run/par/6param/"  #"$HOME/setups/bale2002/test/"  
#unmodpath="/gpfs/work/lie/bale2002/" #"$HOME/setups/bale2002/" 
unmodpath="/gpfs/work/lie/tmzjul/0coagulation_rate50.0/" 
...
while IFS= read -r f; do # IFS= read -r l <&3;
	echo "$f"
done < /gpfs/work/lie/results/paramtochange6.txt #< l.txt
#f=/gpfs/work/lie/results/paramtochange.txt
f=/gpfs/work/lie/results/paramtochange6.txt #therun.txt
...
IFS=$'\n' read -d '' -r -a lines < /gpfs/work/lie/results/paramtochange6.txt
printf '%s\n' "${lines[@]}"
echo "mark here"
echo "${lines[1]}"
...
#for ((i=0; i<n-1; i++)); do 
   ...
   cp $unmodpath"output_more.yaml" output.yaml  #outputall.yaml for all variables
...
python3 /gpfs/work/lie/bale2002/Plot/plottmzjul.py 4 4 4  #postparameter6.py
exit
